...
- **不可用模态的 logits 硬置 $-\infty$**：这是**结构约束**而非正则化。
  ...
  被赋平均 0.287 的 vision 权重（高于全体平均 0.227），输出"主要参考模态 = vision"这种荒谬解释。
...
- batch 128，最多 60 轮，**按验证集 Macro-F1 选最佳 epoch**，连续 12 轮无改善即早停（实测最佳第 8 轮，第 20 轮停）。
...
**(b) 位置→词为按比例映射**：$w(j)=\mathrm{round}\!\big(\frac{j-1}{n}\cdot N\big)$，
其中 $n$ 为该样本的有效位置数、$N$ 为词数。本机无法获取 BERT 分词器，故为**近似**（$n/N\approx1.2$，通常误差 ≤1 词）。
...
> 均匀切分与词级对齐的偏差为**均值 1.968 s、最大 3.627 s**；
...
实测只有 text 满足（76.5%；vision 34.2%、audio 35.7%），且 vision/audio 的低谷组略高于随机组，
...
1. 证据时间的秒级定位来自**自建词级强制对齐**，不是官方时间戳；子词→词为近似。
